...
def plot_best_config_vs_powerlimit_single(best_configs, out_dir="rq2_plots"):
    """
    Creates a unified plot with enhanced readability through:
    - Dual y-axes with shared sequence length color encoding
    - Highlighted extreme values
    - Strategic transparency and line weighting
    - Annotation of optimal points
    """
    if best_configs.empty:
    ...
    seq_lens = sorted(best_configs["seq_len"].unique())
    cmap = plt.cm.viridis
    color_vals = np.linspace(0, 1, len(seq_lens))
    
    # Plotting parameters
    energy_style = {'ls': '-', 'marker': 'o', 'alpha': 0.7, 'lw': 1}
    throughput_style = {'ls': '--', 'marker': 's', 'alpha': 0.7, 'lw': 1}
    
    # Store extremes for annotation
    extremes = {'min_energy': None, 'max_throughput': None}
    ...
    for i, seq in enumerate(seq_lens):
        ...
        energy_line, = ax.plot(subset["nvml_pwr_limit"], subset["Joules/token"],
                              color=color, **energy_style)
        
        # Plot throughput (right axis)
        throughput_line, = ax2.plot(subset["nvml_pwr_limit"], subset["throughput"],
                                   color=color, **throughput_style)
        
        # Track extremes
        if extremes['min_energy'] is None or subset["Joules/token"].min() < extremes['min_energy'][1]:
            extremes['min_energy'] = (seq, subset["Joules/token"].min())
        if extremes['max_throughput'] is None or subset["throughput"].max() > extremes['max_throughput'][1]:
            extremes['max_throughput'] = (seq, subset["throughput"].max())

    # Axis formatting
    ax.set_xlabel("Power Limit (W)", fontsize=12)
    ax.set_ylabel("Joules/token", fontsize=12)
    ax2.set_ylabel("Throughput (tokens/sec)", fontsize=12)
    
    # Set smart axis limits
    ax.set_ylim(0, best_configs["Joules/token"].max() * 1.1)
    ax2.set_ylim(0, best_configs["throughput"].max() * 1.1)
    
    # Create unified legend
    proxy_energy = plt.Line2D([0], [0], color='gray', **energy_style)
    proxy_throughput = plt.Line2D([0], [0], color='gray', **throughput_style)
    ax.legend([proxy_energy, proxy_throughput], 
             ['Energy (Solid)', 'Throughput (Dashed)'],
             loc='upper left')

    # Add colorbar for sequence lengths
    sm = plt.cm.ScalarMappable(cmap=cmap, norm=plt.Normalize(vmin=min(seq_lens), vmax=max(seq_lens)))
    cbar = plt.colorbar(sm, ax=ax, pad=0.02)
    cbar.set_label("Sequence Length", rotation=270, labelpad=15)

    # Annotate extreme values
    ax.annotate(f"Min Energy\n{extremes['min_energy'][0]} tokens",
               xy=(subset["nvml_pwr_limit"].iloc[-1], extremes['min_energy'][1]),
               xytext=(10, -20), textcoords='offset points',
               arrowprops=dict(arrowstyle="->"))
    
    ax2.annotate(f"Max Throughput\n{extremes['max_throughput'][0]} tokens",
                xy=(subset["nvml_pwr_limit"].iloc[-1], extremes['max_throughput'][1]),
                xytext=(-80, 20), textcoords='offset points',
                arrowprops=dict(arrowstyle="->"))

    plt.title("Energy vs Throughput Tradeoffs Across Power Limits\n"
             "Color = Sequence Length | Solid = Energy | Dashed = Throughput",
             pad=20, fontsize=14)
    
    plt.tight_layout()
    outpath = os.path.join(out_dir, "power_tradeoffs_enhanced.png")
    plt.savefig(outpath, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Saved enhanced plot: {outpath}")
...
def generate_text_report(df, best_configs, best_by_seq, out_dir="rq2_plots"):
    ...
    with open(report_file, "w") as f:
        ...
        for seq in sorted(df["seq_len"].unique()):
            ...
            throughput_baseline = float(baseline_row["throughput"])
            throughput_best = float(best_row["throughput"])
            throughput_change = throughput_best - throughput_baseline
            pct_throughput_change = (throughput_change / throughput_baseline) * 100 if throughput_baseline > 0 else 0.0
            ...
            f.write(f"  Throughput change: {throughput_change:+.2f} tokens/s ({pct_throughput_change:+.1f}%)\n\n")
...
def main():
    ...
    # Generate a unified plot for all sequence lengths.
    plot_best_config_vs_powerlimit_single(best_configs, out_dir=out_dir)
