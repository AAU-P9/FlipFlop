        ...
        if arg_type == "array":
            ...
            if isinstance(val, str):
                try:
                    ...
                    raise ValueError(f"Error converting float value '{val}': {e}")
...
def main():
    ...
    # parser.add_argument("--norun", action="store_true", help="Skip actual kernel execution; only do static analysis with Hong–Kim model.")
    ...
    warps_per_sm = time_model._calc_blocks_per_sm(args.bdimx * args.bdimy) * math.ceil((args.bdimx*args.bdimy)/arch.attrs['WARP_SIZE'])
    active_sms = arch.sm_count

    # Predict power using the Hong–Kim power model
    power_estimator = HongKimPowerEstimator(arch, analysis)
    predicted_power = power_estimator.estimate_power(exec_cycles, warps_per_sm, arch.sm_count, arch.clock_rate_hz)
