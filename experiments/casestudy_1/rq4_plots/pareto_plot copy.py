...
# Plot all configurations in light gray
ax.scatter(energy, throughput, color='lightgray', s=20, edgecolor="gray", label='All configurations')

# Plot Pareto optimal points as red markers
ax.scatter(e_pareto, t_pareto, color='red', s=80, edgecolor="black" ,label='Pareto optimal')

# Smooth interpolation for the Pareto front curve
if len(e_pareto) >= 4:
    e_smooth = np.linspace(e_pareto.min(), e_pareto.max(), 300)
    try:
        spline = make_interp_spline(e_pareto, t_pareto, k=3)
        t_smooth = spline(e_smooth)
        ax.plot(e_smooth, t_smooth, color='red', linewidth=2, label='Pareto front')
    except Exception as ex:
        print("Interpolation failed:", ex)
        ax.plot(e_pareto, t_pareto, '-o', color='red', linewidth=2, label='Pareto front')
else:
    ax.plot(e_pareto, t_pareto, '-o', color='red', linewidth=2, label='Pareto front')

# Annotate each Pareto point with its configuration (block@power)
for config in pareto_data:
    block, power, en, thr = config
    ax.text(en, thr, f"{int(block)}@{int(power)}", fontsize=8, va='bottom', ha='right')

ax.set_xlabel('Energy per Token (μJ, lower is better)', fontsize=12)
ax.set_ylabel('Throughput (tokens/s, higher is better)', fontsize=12)
ax.set_title('Pareto Front: Energy vs. Throughput', fontsize=14)
ax.grid(True)
ax.legend(loc='lower left', fontsize=10)

# Create an inset zoom on the Pareto region
x_min, x_max = e_pareto.min() * 0.98, e_pareto.max() * 1.02
y_min, y_max = t_pareto.min() * 0.98, t_pareto.max() * 1.02
axins = inset_axes(ax, width="40%", height="40%", loc='upper right')
axins.scatter(energy, throughput, color='lightgray', s=20)
axins.scatter(e_pareto, t_pareto, color='red', s=80)
if 't_smooth' in locals():
    axins.plot(e_smooth, t_smooth, color='red', linewidth=2)
else:
    axins.plot(e_pareto, t_pareto, '-o', color='red', linewidth=2)
