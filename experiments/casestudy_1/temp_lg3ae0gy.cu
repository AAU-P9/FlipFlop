...
// Power management stages
typedef enum {
    STAGE_INIT_MEMORY,      // Memory allocation/data transfers
    STAGE_ATTN_PREP,        // QKV projections/RoPE
    STAGE_ATTN_CORE,        // Attention scores/softmax
    STAGE_ATTN_OUTPUT,      // Attention output projection
    STAGE_FFN_SILU,         // FFN SwiGLU activation
    STAGE_FFN_OUTPUT,       // FFN output projection
    STAGE_RESIDUAL,         // Residual connections
    STAGE_FINAL_NORM       // Final RMSNorm
} PowerStage;
...
extern "C" __global__ void rmsnorm_kernel(float *o, float *x, float *weight, int size, int elementsPerThread) {
...
extern "C" __global__ void matmul_kernel(float *xout, float *x, float *w, int n, int d) {
...
extern "C" __global__ void RoPe_rotation_kernel(int pos, float *sq, float *sk, int kv_dim, int head_size) {
    int i = threadIdx.x * 2;
...
extern "C" __global__ void multi_head_attention_kernel(int pos, int seq_len, float *sq, float *satt, float *sxb, float *key_cache,
...
extern "C" __global__ void f_silu_elementwise_mul_w3_kernel(float *shb, float *shb2, int hidden_dim) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < hidden_dim) {
        float val = shb[i];
...
extern "C" __global__ void accum_kernel(float *a, float *b, int size) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < size) {
        a[i] += b[i];
    }
...
//            transformer.config.n_layers, transformer.config.n_heads, transformer.config.dim);
