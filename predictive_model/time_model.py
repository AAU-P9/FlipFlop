...
class HongKimExecutionTimeModel:
    ...
    def estimate_time_ns(self) -> float:
        ...
        # Coalescing efficiency based on blockDim.x relative to warp size
        ce_x = min(1.0, block_dim_x / warp_size)
        
        # Shape balance factor: penalizes extreme aspect ratios
        aspect_ratio = block_dim_x / block_dim_y if block_dim_y > 0 else 1.0
        shape_balance = 1.0 + 0.2 * abs(math.log(max(aspect_ratio, 1e-6)))  # Avoid log(0)
        
        # Combined shape factor: increases cycles for less efficient shapes
        shape_factor = shape_balance / ce_x if ce_x > 0 else shape_balance
        shape_factor = max(1.0, min(shape_factor, 2.0))  # Clamp between 1.0 and 2.0
