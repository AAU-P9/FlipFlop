...
def plot_actual_time_and_power(csv_file):
    ...
    for kernel in kernels:
        ...
        # Create a figure with two subplots (Actual Time and Actual Power)
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 8), sharex=True)

        # Plot Actual Time (ns)
        ax1.plot(x, df_kernel['ActTime(ns)'], marker='o', linestyle='-', color='blue', label='Actual Time')
        ax1.set_ylabel("Actual Time (ns)")
        ax1.set_title(f"{kernel} - Actual Time vs. Block Configuration")
        ...
        ax2.set_title(f"{kernel} - Actual Power vs. Block Configuration")
        ax2.set_xticks(x)
        ...
        outname = f"plots/{kernel}_actual_time_power.png"
...
def main():
    ...
    plot_actual_time_and_power(csv_file)
