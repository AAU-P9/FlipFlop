                        ...
                        const int n, const int d, const int impl_num, const int repeat, std::vector<KernelRun>& kernel_runs)
{
  ...
  if (impl_num == 3) {
    ...
    for (int k = 0; k < repeat; k++) {
      ...
      LAUNCH_KERNEL(kernel1_warpReduce, dim3((n+7)/8), dim3(256), d_key, d_query, d_dot_product, d_exp_sum, n, d);
      LAUNCH_KERNEL(kernel2_blockReduce, dim3(d), dim3(256), d_exp_sum, d_dot_product, d_value, d_output, n, d);
    }
    ...
    for (int k = 0; k < repeat; k++) {
      cudaMemset(d_exp_sum, 0, 4);
      LAUNCH_KERNEL(kernel1_warpReduce,dim3((n+7)/8), dim3(256), d_key, d_query, d_dot_product, d_exp_sum, n, d);
      LAUNCH_KERNEL(kernel2_warpReduce,dim3((d+7)/8), dim3(256), d_exp_sum, d_dot_product, d_value, d_output, n, d);
    }
    ...
    for (int k = 0; k < repeat; k++) {
      cudaMemset(d_exp_sum, 0, 4);
      LAUNCH_KERNEL(kernel1_blockReduce,dim3(n) ,dim3(256) , d_key, d_query, d_dot_product, d_exp_sum, n, d);
      LAUNCH_KERNEL(kernel2_blockReduce,dim3(d) ,dim3(256) , d_exp_sum, d_dot_product, d_value, d_output, n, d);
    }
    ...
    for (int k = 0; k < repeat; k++) {
      cudaMemset(d_exp_sum, 0, 4);
      LAUNCH_KERNEL(kernel1,dim3((n+255)/256), dim3(256), d_key, d_query, d_dot_product, d_exp_sum, n, d);
      LAUNCH_KERNEL(kernel2,dim3((n+255)/256), dim3(256), d_exp_sum, d_dot_product, d_score, n);
      LAUNCH_KERNEL(kernel3,dim3((d+255)/256), dim3(256), d_score, d_value, d_output, n, d);
    }
...
int main(int argc, char* argv[]) {
  ...
  #ifdef USE_NVML
  PowerProfiler profiler(kernel_runs);
  #endif
  ...
  float* dout = attention_device(key, value, query, n, d, k, r, kernel_runs);

  #ifdef USE_NVML
  char filename[256];
  snprintf(filename, sizeof(filename), "power_data_%dx%d_impl%d.csv", n, d, k);
